...
public class Commands {
	...
	public static class set_tabcompleter extends AbstractFunction {
		...
		public static void customExec(Target t, Environment environment, MCCommand cmd, Construct arg) {
			if (arg instanceof CClosure) {
				...
				cmd.setTabCompleter(Static.getServer().getPluginManager().getPlugin(Implementation.GetServerType().getBranding()));
			} else {
		...
		@Override
		public String docs() {
			return "void {commandname, closure} Sets the code that will be run when a user attempts to tabcomplete a command.";
		}
	...
	public static class set_executor extends AbstractFunction {
		...
		public Construct exec(Target t, Environment environment, Construct... args) throws ConfigRuntimeException {
			...
			customExec(t, environment, cmd, args[0]);
			return new CVoid(t);
		}
